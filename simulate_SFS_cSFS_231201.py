...
parser.add_argument('-T_supersuper_archaic','--T_supersuper_archaic',help='(model A and Cprime) split time in years of root of (T_super_archaic,super_ghost)',required=False,type=float)
parser.add_argument('-T_pulse_superghost_to_DEN','--T_pulse_superghost_to_DEN',help='(model A and Cprime) time in years at which superghost lineage introgresses into Denisovans',required=False,type=float)
...
parser.add_argument('-N_supersuper_ancestral','--N_supersuper_ancestral',help='(model A and Cprime) Population size of super super ancestral branch',required=False,type=float)
parser.add_argument('-N_superghost','--N_superghost',help='(model A and Cprime) Population size of superghost lineage (the one that introgresses into Neanderthal)',required=False,type=float)
...
parser.add_argument('-N_ghost_recent','--N_ghost_recent',help='(model C) Population size of ghost lineage after (forwards in time) it introgresses into Neanderthal but before it introgresses into MH',required=False,type=float)
...
if (T_AMH_expand==None and N_AMH_present!=None) or (T_AMH_expand!=None and N_AMH_present==None):
    print(f'ERROR: Either give both T_AMH_expand and N_AMH_present, or neither. Aborting.')
    sys.exit()
elif T_AMH_expand==None and N_AMH_present==None:
...
elif (T_ghost_BN_start!=None and T_ghost_BN_end!=None and N_ghost_BN_intensity!=None):
    if T_ghost_BN_start<=T_ghost_BN_end:
        print(f'ERROR: T_ghost_BN_start must be greater than T_ghost_BN_end. Aborting')
        sys.exit()
else:
